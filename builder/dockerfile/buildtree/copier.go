...
type copier struct {
	imageSource builder.ImageMount
...
func copierFromContext(ctx *buildContext, download sourceDownloader, imageSource builder.ImageMount) copier {
	return copier{
		source:      ctx.source,
...
func (o *copier) calcCopyInfo(origPath string, allowWildcards bool) ([]copyInfo, error) {
	...
	if imageSource != nil && imageSource.Image().ImageID() != "" {
		// return a cached copy if one exists
		if h, ok := o.pathCache.Load(imageSource.Image().ImageID() + origPath); ok {
...
func (o *copier) storeInPathCache(im builder.ImageMount, path string, hash string) {
	if im != nil {
		o.pathCache.Store(im.Image().ImageID()+path, hash)
	}
}
